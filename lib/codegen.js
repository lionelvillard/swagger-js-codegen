...
var getViewForSwagger2 = function(opts, type){
    ...
    _.forEach(swagger.paths, function(api, path){
        ...
        _.forEach(api, function(op, m){
            ...
            if(consumes) {
                // consumes is an array. Get the first item.
                if (consumes.length > 0)
                    method.headers.push({name: 'Content-Type', value: '\'' + consumes[0] + '\'' });
            }
                ...
                if(parameter.in === 'body'){
                    ...
                    if (parameter.hasOwnProperty('schema')) {
                      ...
                      if (parameter.schema.hasOwnProperty('$ref')) {
                        // A reference to schema. Support local reference only.
                        var ref = parameter.schema['$ref'];
                        var idx = ref.indexOf('definitions/');
                        if (idx > 0) {
                           id = ref.substring(idx + 12);
                           if (id.length > 0)
                            schema = swagger.definitions[id];
                        }
                      } else {
                      ...
                      if (schema) {
                        addSchemaParams('body', '', schema, parameter.required, true, method.parameters);
                      }
                    }
...
var getCode = function(opts, type) {
    ...
    if (type === 'custom') {
        ...
        if(type === 'typescript') {
          opts.template.method = opts.template.method || fs.readFileSync(templates + 'typescript-method.mustache', 'utf-8');
          opts.template.type = opts.template.type || fs.readFileSync(templates + 'type.mustache', 'utf-8');
        } else if(type === 'openwhisk') {
          opts.template.method = opts.template.method || fs.readFileSync(templates + 'openwhisk-method.mustache', 'utf-8');
        } else {
    ...
    var lintOptions = {
        node: type === 'node' || type === 'custom' || type == 'openwhisk',
    ...
    if(type === 'typescript' || type === 'openwhisk') {
        opts.lint = false;
    }
...
// Split Openwhisk code into individual actions
var splitCode = function(opts, code) {
  var actions = code.split('/*$OWACTIONBBEGING$*/');
  actions = actions.slice(1);
  var result = [];
  actions.forEach((action, index) => {
    var closing = action.indexOf('*/');
    var name = action.substring(3, closing);
    var code = action.substring(closing + 2);

    if (opts.lint === undefined || opts.lint === true) {
        lint(code,  {node:true, trailing:true, smarttabs:true, maxerr:999});
        lint.errors.forEach(function(error) {
            if (error.code[0] === 'E') {
                throw new Error(error.reason + ' in ' + error.evidence + ' (' + error.code + ')');
            }
    ...
    if (opts.beautify === undefined || opts.beautify === true) {
       code = beautify(code, { indent_size: 4, max_preserve_newlines: 2 });
    }
...
var addSchemaParams = function(basename, name, schema, required, isBodyParameter, params) {
  var param = {
    camelCaseName: (name.length > 0 ) ? `${basename}.${name}` : basename,
    description: schema.description,
    required: required,
    isSchemaParameter: true,
  ...
  if (schema.hasOwnProperty('properties')) {
    basename = (name.length > 0 ) ? `${basename}.${name}` : basename;
    var required = schema.required || [];
    for (var propName in schema.properties) {
      if (schema.properties.hasOwnProperty(propName)) {
        addSchemaParams(basename, propName, schema.properties[propName], required.find(req => propName === req), false, params);
      }
    }
  }
}
...
exports.CodeGen = {
    ...
    getOpenWhiskCode: function(opts){
        return splitCode(opts, getCode(opts, 'openwhisk'));
    },
